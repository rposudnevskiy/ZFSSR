...
class Implementation(Plugin_skeleton):
    ...
    def query(self, dbg):
        if platform.linux_distribution()[1] == '7.5.0':
            return {
                "plugin": "zfs+raw+qdisk",
                "name": "The ZFS ZVol QEMU qdisk user-space datapath plugin",
                "description": ("This plugin manages and configures qdisk"
                                " instances backend for zvol image in raw format built"
                                " using libzfs"),
            ...
            return {
                "plugin": "zfs+raw+qdisk",
                "name": "The ZFS ZVol qdisk user-space datapath plugin",
                "description": ("This plugin manages and configures qdisk"
                                " instances backend for zvol image in raw format built"
                                " using libzfs"),
                "vendor": "Roman V. Posudnevskiy",
